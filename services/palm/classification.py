...
def classify(path_to_palmline_image):
    ...
    # Load the palmline image
    palmline_img = cv2.imread(path_to_palmline_image)

    # Check if image is loaded properly
    if palmline_img is None:
        raise ValueError(f"Image not found or invalid format: {path_to_palmline_image}")
    
    kernel = np.ones((3, 3), np.uint8)

    # Skeletonize the image (convert to grayscale first)
    gray_img = cv2.cvtColor(palmline_img, cv2.COLOR_BGR2GRAY)
    skeleton = skeletonize(gray_img > 0)  # Skeletonize expects a binary image
